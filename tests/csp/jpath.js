var jpath = require('../../lib/csp/jpath');
...
it('It works', function() {
    expect(j('.hello2.a', [[[{ hello2: {a: [11]} }]]] )).to.be.eql([ [ 11 ] ]);
    expect(j('.*', {a: [{a: 10, b:11}], b: [{b: 10, a:11}]})).to.be.eql([ [ { a: 10, b: 11 } ], [ { b: 10, a: 11 } ] ]);
    expect(j('.*', 'hello')).to.be.eql([ 'h', 'e', 'l', 'l', 'o' ]);
    expect(j('.a', [ [ { a: 10, b: 11 } ], [[ { b: 10, a: 11 } ]] ])).to.be.eql([ 10, 11 ]);
    expect(j('.*', [ [ { a: 10, b: 11 } ], [ { b: 10, a: 11 } ] ])).to.be.eql([ 10, 11, 10, 11 ]);
    expect(j('.*.*.a', { a: [{ a: [{a: 11}], b: {}}], b: { a: {}, b: {a:[12]}}})).to.be.eql([ 11, [ 12 ] ]);
    expect(j('.a[.f]', { a: [{b: 11, f: true}, {b: 12, f: false}, {b: 13, f: true}] })).to.be.eql([ { b: 11, f: true }, { b: 13, f: true } ]);
    expect(j('.*[.f]', { a: [{b: 11, f: true}, {b: 12, f: false}, {b: 13, f: true}] })).to.be.eql([ { b: 11, f: true }, { b: 13, f: true } ]);
    expect(j('.hello2.a[1].email', { hello2: {a: [ 10, { email: 11 } ]} } )).to.be.eql([ 11 ]);
    expect(j('.hello2.a[2]', [ { hello2: {a: [ [[10]], [[11]] ]} }, { hello2: {a: [ [[12]] ]} } ] )).to.be.eql(undefined);
    expect(j('.hello2.a[2]', { hello2: [{a: [ [[10]], [[11]] ]}] } )).to.be.eql(undefined);
    expect(j('.hello2.a[1]', { hello2: {a: [ 10, [[[0]]] ]} } )).to.be.eql([ [[[0]]] ]);
    expect(j('.a[0][0]', { a:[ [ 'hello' ] ] } )).to.be.eql(undefined);
    expect(j('.a.email.ref', { a: [ { email: [{ref: 'bb'}, 'aa'] } ], b: ['bb','bb'] })).to.be.eql(['bb']);
    expect(j('5 + .a.c[1].b + .b + 5 + "hello"', { a: { c: [ [{b:11}] ] }, b: 11 })).to.be.eql('5115hello');
    expect(j('.a[.email.ref == /.b[0] ].email.ref', { a: [ { email: [{ref: 'bb'}, 'aa'] } ], b: ['bb','bb'] } )).to.be.eql(['bb']);
    expect(j('.a[.email[0].ref == /.b[0] ].email.ref', { a: [ { email: [{ref: 'bb'}, 'aa'] } ], b: ['bb','bb'] } )).to.be.eql(['bb']);
    expect(j('.a[.email[0].ref != .b[0] ].email.ref', { a: [ { email: [{ref: 'bb'}, 'aa'], b: ['cc'] } ], b: ['bb','bb'] } )).to.be.eql(['bb']);
    expect(j('.a[.email[0].ref != .b[0] ].email.ref', { a: [ { email: [{ref: 'bb'}, 'aa'], b: ['bb'] } ], b: ['bb','bb'] } )).to.be.eql(undefined);
    expect(j({a: '.a', b: '.a.b'}, { a: [ { b: 11 } ]})).to.be.eql({a: [{b:11}], b: [ 11 ]});
    expect(j({a: '.a', b: '.a.b'}, { b: [ { b: 11 } ]})).to.be.eql({a: undefined, b: undefined});
    expect(j('.a.c[0].b + .b + "hello"', { a: { c: [ [{b:11}] ] }, b: 11 })).to.be.eql('22hello');
    expect(j('(---(5 + 6) + 2 * 2) == -7')).to.be.eql(true);
});
